...
the system allowed to do next", so that decision stays reviewable and testable
on its own.

Chunk 03 wrote this under `tools/telnyx_probe/` because there was nothing to
wire it to yet. Chunk 15 is that wiring: `ConnectivityService.reconcile_attempt`
calls it with the result of a real tag lookup, so the module moved into the
application unchanged rather than being reimplemented next to it.
