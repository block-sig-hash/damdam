...
describe('pinLocalStore', () => {
  it('saves a fresh PIN with zeroed attempts and no lock', async () => {
    ...
    expect(mockSet).toHaveBeenCalledWith(
      'pin',
      JSON.stringify({ pin: '4682', failedAttempts: 0, lockedUntil: null }),
  ...
  it('reports no PIN stored when the keychain has nothing (AC-23.4 new-device case)', async () => {
    ...
    expect(await hasPinStoredLocally()).toBe(false);
    expect(await getLocalPinState()).toBeNull();
  });

  it('verifies a matching PIN locally and resets the failure counter (AC-02.3/AC-23.3)', async () => {
    mockGet.mockResolvedValue(
      credentialsFor({ pin: '4682', failedAttempts: 3, lockedUntil: null }),
    );
    mockSet.mockResolvedValue(false);

    expect(await verifyPinLocally('4682')).toBe(true);
    expect(mockSet).toHaveBeenCalledWith(
      'pin',
      JSON.stringify({ pin: '4682', failedAttempts: 0, lockedUntil: null }),
      expect.anything(),
  ...
  it('rejects a non-matching PIN without writing anything', async () => {
    mockGet.mockResolvedValue(
      credentialsFor({ pin: '4682', failedAttempts: 0, lockedUntil: null }),
    );

    expect(await verifyPinLocally('9317')).toBe(false);
  ...
  it('does not lock before the 5th failed attempt', async () => {
    mockGet.mockResolvedValue(
      credentialsFor({ pin: '4682', failedAttempts: 1, lockedUntil: null }),
    );
    mockSet.mockResolvedValue(false);
