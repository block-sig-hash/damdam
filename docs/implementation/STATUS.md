...
| 17 — Implement top-ups, spending controls and suspension | **READY_FOR_REVIEW**; the hard prepaid guarantee is **BLOCKED** | accepted chunk 16 dependency `ad7a267` / corrected `c48e58d` / see [handoff](handoffs/17.md) | [Independent review in progress](reviews/17.md) | **D1 OPEN** — Telnyx documents no enforcement latency and no voice cap, so `prepaid_guarantee` returns false and a shared carrier/internet credit pool is refused. **D5 OPEN** — the bounded-exposure alternative needs an approval |
