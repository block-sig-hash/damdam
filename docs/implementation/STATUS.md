...
| V04 — Mobile outbound | US-47 | **ACCEPTED after fixes** (provider-neutral client scope); SDK integration and live call **EXTERNAL_BLOCKED** | submitted `5714aa4` / replayed `b7b82e3` / corrected `01ce544` / [review](reviews/V04.md), [handoff](handoffs/V04.md) — lifecycle closure, installation-scoped credentials and a true pre-hold identity preview are verified. **D1 and B1–B5 OPEN**: no Telnyx account, SDK, native build, live audio, DTMF, interruption or cost evidence. `RECORD_AUDIO` remains deliberately absent and chunk 04's retirement guard is untouched |
| V05 — Browser outbound | US-48 | **READY_FOR_REVIEW** (browser client scope); SDK and live audio **EXTERNAL_BLOCKED** | submitted `PENDING_HEAD` / [handoff](handoffs/V05.md) — `/calls` is a consumer area with its own credential; operator and admin tokens open nothing. Refresh, duplicate tabs and sleep reconcile the durable attempt instead of redialling. **D1 and B1–B5 OPEN**: no Telnyx account, no SDK, no live browser audio, DTMF or charge evidence. Browser origin policy and rotation remain chunks 25–26 |
