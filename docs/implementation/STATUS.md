...
| 16 — Implement actual usage reconciliation and carrier charging | **ACCEPTED after fixes** (carrier documented/fixture scope); live evidence EXTERNAL_BLOCKED | accepted base `b7f5b68` / replayed `211c00d` / corrected `7bf87f0` | [Independent review](reviews/16.md) | **D1/D5 OPEN** — no supplier usage observed; `usage_events` remains withheld and no WDR file is parsed. Exhaustion enforcement is chunk 17; internet-call metering is V03 |
| 17 — Implement top-ups, spending controls and suspension | **READY_FOR_REVIEW**; the hard prepaid guarantee is **BLOCKED** | base chunk 16 tip (**unaccepted**) / see [handoff](handoffs/17.md) | Not reviewed | **D1 OPEN** — Telnyx documents no enforcement latency and no voice cap, so `prepaid_guarantee` returns false and a shared carrier/internet credit pool is refused. **D5 OPEN** — the bounded-exposure alternative needs an approval |
