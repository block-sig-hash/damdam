...
| 15 — Implement the Telnyx eSIM, line and number lifecycle | **READY_FOR_REVIEW** (documented scope); live carrier scope EXTERNAL_BLOCKED | base `c945c52` (chunk 14 tip, **unaccepted**) / see [handoff](handoffs/15.md) | Not reviewed | **D1 OPEN** — no Telnyx account, no call made, no rate deck. Carrier voice, number assignment, top-up, usage events and spending enforcement are withheld capabilities with recorded reasons |
