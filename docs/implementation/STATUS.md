...
| V04 — Mobile outbound | US-47 | **READY_FOR_REVIEW** (client scope); SDK integration and live call **EXTERNAL_BLOCKED** | submitted `PENDING_HEAD` / [handoff](handoffs/V04.md) — Calls tab, calling client, provider boundary and session controller are built against V02/V03's accepted contract. **D1 and B1–B5 OPEN**: no Telnyx account, no SDK installed, no native build, no live audio, DTMF, interruption or cost evidence. `RECORD_AUDIO` remains deliberately absent and chunk 04's retirement guard is untouched |
