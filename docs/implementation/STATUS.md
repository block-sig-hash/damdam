...
Chunks 01–22 are **ACCEPTED after independent Codex review and fixes** for the
...
| 22 — Build enterprise people, teams and validated imports | **ACCEPTED after fixes** (people/import/access-view scope; live login delivery preparatory) | accepted base `f2f0f7b` / replayed `584d546` / corrected `99afb5e` | [Independent review](reviews/22.md) | Passwordless dashboard login is wired to the identity API, but the configured delivery transport discards email outside tests. Membership/invitation views are read-only; privileged mutations remain backend/MFA flows |
