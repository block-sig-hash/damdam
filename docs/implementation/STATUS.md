...
| 26 — Harden environments, backups and operational visibility | **READY_FOR_REVIEW (26A–C only)**; 26D not started | submitted `PENDING_HEAD` | [handoff](handoffs/26.md) | Delivered: correlation ids, redaction-safe structured logs, production configuration refusals, browser-origin policy and the operational metrics endpoint. **Open (26D):** staging/production configuration with isolated databases and least-privilege credentials, worker shutdown/retry/alert ownership, the backup and restore drill with recorded recovery time, migration rollback, and secret rotation |
