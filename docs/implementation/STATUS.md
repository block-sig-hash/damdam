...
| 19 — Build the complete consumer purchase journey | **READY_FOR_REVIEW**; live payment **EXTERNAL_BLOCKED** | base `df81e71` (chunk 18 tip, **unaccepted**) / head `PENDING_HEAD` / see [handoff](handoffs/19.md) | Not reviewed | **D4 OPEN** — no processor is selected, so `payment_processor_adapter` is `None`, every checkout returns `awaiting_processor` and no payment can complete. **D3 OPEN** — no live merchant account, so `collection_enabled` is false and `tax_configuration_reference` is null everywhere. **D2** — no market is published, so a real catalog is empty. The journey, its idempotency and its seven recovery paths are built and tested against those answers rather than around them. Screenshot flows are committed; the PNGs are CI's to capture |
