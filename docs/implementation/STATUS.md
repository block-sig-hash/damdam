...
| 20 — Build My Line, eSIM installation and native-call guidance | **READY_FOR_REVIEW**; live installation and carrier voice **EXTERNAL_BLOCKED** | base `a6ef588` (develop, chunk 19 merged) / head `PENDING_HEAD` / see [handoff](handoffs/20.md) | Not reviewed | **D1 OPEN** — no Telnyx account, no provisioned line, no activation material, so no profile has ever been installed on a handset. `ACTIVATION_MATERIAL_KEY` is unset in every environment, which makes delivery fail closed with `material_key_unavailable` by design. Chunks 15–17 had no HTTP surface or app wiring at all; this chunk adds both. No QR image is rendered (the vault stores an LPA string, not a supplier QR URL) and `ScreenPrivacyModule.kt` has not been compiled here — CI's Android build is its first check |
