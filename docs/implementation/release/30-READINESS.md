...
|---|---|---|
| Independent findings and exact-head checks | Chunk 27–30 preparatory reviews accepted; [chunk 30 exact-head CI](https://github.com/block-sig-hash/damdam/actions/runs/36025079138) is red, and combined journeys/cloud signing compatibility remain open | Engineering owner closes findings, reruns affected checks on final SHA, records review disposition and CI run IDs |
| WAL-G CI test dependency | **BLOCKED** on chunk 30 CI: pinned Quay MinIO images return `unauthorized`, so the PITR job is not green. A local cached-image drill passed (`PITR_PROOF=PASS`, `PITR_RECONCILIATION=PASS`, 10-second recovery) but does not repair CI or prove production restore | CI/operations owner restores a trusted reproducible S3-compatible fixture and reruns the real archive/PITR drill |
...
| Automated native CI | On chunk 30 documentation head `6e3446d` containing reviewed code `c9d110a`, both Android emulator and iOS simulator passed 36/36 flows and 72/72 PNGs. This is not a later frozen release-candidate SHA | CI owner reruns both jobs on the exact frozen candidate, records artifact IDs and compares source/configuration |
...
revision must equal the tested source commit's single connected, acyclic
Alembic head. Operator references are opaque roster IDs; the restricted
ownership evidence binds them to real named people and routes. Its Ed25519
