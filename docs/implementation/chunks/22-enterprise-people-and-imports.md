# Chunk 22 — Build enterprise people, teams and validated imports
...
**Depends on:** chunk 07, chunk 08, chunk 09.
**Plan mapping:** PR-13 / US-39.
**Likely starting points:** Existing manifests/dashboard pages; organization permissions and frontend dashboard spec.
Paths refer to the inspected baseline; verify them in the actual current checkout.

## Calling expansion requirements

Keep consumer calling access separate from enterprise administrative roles. Model work-service eligibility without assuming every member owns an eSIM.

## Outcome and implementation scope

1. Replace tour-operator assumptions with organization people, teams/departments, cost centers and membership/invitation views.
2. Adapt CSV import to validate per row, preview changes and report errors; define duplicate identity handling, limits and resumable import records.
3. Separate service recipients from dashboard administrators; importing an email must not grant organization privilege.
4. Implement UI authorization plus mandatory backend enforcement. Respect tenant scope in downloads and bulk selections.
5. Generalize government as an organization category without claims of certification or automatic additional data access.

## Acceptance and evidence

- Test duplicate/replayed imports, invalid rows, CSV formula injection in exports, oversized inputs, recipient collision and cross-tenant access.
- Render English/French desktop/mobile-width states and verify billing/member roles cannot perform administrator actions.
