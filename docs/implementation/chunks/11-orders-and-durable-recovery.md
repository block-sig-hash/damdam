# Chunk 11 — Build order orchestration and durable recovery
...
**Depends on:** chunk 09, chunk 10.
**Plan mapping:** PR-06 part B / US-32; finding 4.
**Likely starting points:** esim/service.py, provider errors, payment fulfillment, worker scheduling and order models.
Paths refer to the inspected baseline; verify them in the actual current checkout.

## Calling expansion requirements

Consume V01 for voice-specific entitlement mapping. Internet-only purchases must not trigger eSIM provisioning. Preserve the quantity-one recipient item invariant and immutable payer/seller/currency.

## Outcome and implementation scope

1. Implement item-level orders and separate payment/provisioning states with durable outbox/inbox processing and idempotent consumers.
2. Persist supplier attempt identity and request reference before dispatch. Distinguish definite rejection, accepted/pending and outcome unknown.
3. On timeout or crash after possible acceptance, reconcile the original supplier. Reuse only documented idempotency keys; if lookup is insufficient, hold for operations review.
4. Do not automatically buy from another supplier or create a fresh order merely because the first response was lost. Preserve safeguards in retained legacy paths.
5. Add bounded retries, retry scheduling, worker claim/lease recovery and cancellation rules across financial and supplier states.

## Acceptance and evidence

- Simulate supplier acceptance followed by lost response and process crash, then restart workers: at most one purchased profile and one accounting effect.
- Test concurrent fulfillment, duplicate outbox delivery, failures between database commit and external calls, poison jobs and cancel-versus-fulfill races.
