# Chunk 10 — Build the multi-currency ledger and atomic reservations
...
**Plan mapping:** PR-06 part A / US-32.
**Likely starting points:** Existing balance/payment accounting; data-model and financial acceptance criteria.
Paths refer to the inspected baseline; verify them in the actual current checkout.

## Calling expansion requirements

Before finalizing voice reservation contracts, consume V01 documented scope. Expose atomic reservation/extension/settlement primitives for V02/V03; multiple channels use one currency-specific ledger, with bounded carrier exposure.

## Outcome and implementation scope

1. Implement immutable balanced journal entries and currency-specific accounts for funding, reservations, consumption, release and compensating adjustments.
2. Define units, exact precision, rounding and unique business-event identifiers. Do not add unrelated currency balances or edit posted history.
3. Enforce atomic availability checks and reservations under concurrent operations. Define lock ordering and retry semantics.
4. Backfill/migrate historical balances with explicit provenance and reconciliation reports, avoiding unsupported assumptions about legacy rows.
5. Keep connectivity service credit closed to DamDam purchases; no transfers, cash-out or unsolicited foreign-exchange engine.

## Acceptance and evidence

- Use PostgreSQL concurrency tests to prove duplicate/reordered postings and simultaneous reservations cannot double-credit or overspend.
- Assert each posted journal balances per currency; verify partial release, compensation, replay and legacy reconciliation. Test failures inside a transaction leave no partial posting.
