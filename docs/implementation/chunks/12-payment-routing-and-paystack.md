# Chunk 12 — Implement payment routing and modernize Paystack
...
**Depends on:** chunk 09, chunk 10, chunk 11.
**Plan mapping:** PR-07 part A / US-33.
**Likely starting points:** payments/providers.py, service/routes/schemas; merchant decision register; payment and security specs.
Paths refer to the inspected baseline; verify them in the actual current checkout.

## Calling expansion requirements

Evaluate routing and processor eligibility for the expanded internet-voice offering as well as eSIM plans. D3/D4 are still open; no processor is selected by this amendment.

## Outcome and implementation scope

1. Introduce a provider-neutral payment contract with immutable seller, merchant account, currency, quote/order and payment-attempt references.
2. Route eligible approved local NGN transactions to Paystack using seller/merchant/currency/method rules, not nationality or IP alone.
3. Modernize the Paystack adapter with hosted/tokenized checkout, webhook verification, status reconciliation and preserved legacy references.
4. Validate authoritative amount, currency, account, order and final status before fulfillment. Browser/app return URLs cannot mark an order paid.
5. Treat unknown charge outcomes as reconciliation work; do not route another charge to a different processor automatically. Keep live collection disabled pending D3/D4.

## Acceptance and evidence

- Test forged/replayed/reordered webhooks, wrong seller/merchant/currency/amount, retry races and redirect spoofing.
- Prove one business order cannot be credited or fulfilled twice, including two successful payment attempts; record/resolve excess payment without losing it.
